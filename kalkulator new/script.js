...
function resetZoom() {
    currentZoom = 1;
    applyZoom();
    appCanvas.scrollLeft = (canvas.offsetWidth * currentZoom - appCanvas.offsetWidth) / 2;
    appCanvas.scrollTop = 0;
}
...
appCanvas.addEventListener('mousedown', (e) => {
    if (e.target.closest('.node') || e.target.closest('.btn')) return;
    isPanning = true;
    appCanvas.classList.add('panning');
    startX = e.pageX - appCanvas.offsetLeft;
    startY = e.pageY - appCanvas.offsetTop;
    scrollLeft = appCanvas.scrollLeft;
    scrollTop = appCanvas.scrollTop;
});

appCanvas.addEventListener('mouseleave', () => {
    isPanning = false;
});
...
appCanvas.addEventListener('mousemove', (e) => {
    if (!isPanning) return;
    e.preventDefault();
    const x = e.pageX - appCanvas.offsetLeft;
    const y = e.pageY - appCanvas.offsetTop;
    const walkX = (x - startX) * 1.5;
    const walkY = (y - startY) * 1.5;
    appCanvas.scrollLeft = scrollLeft - walkX;
    appCanvas.scrollTop = scrollTop - walkY;
});
...
appCanvas.addEventListener('touchstart', (e) => {
    ...
    isPanning = true;
    startX = touch.pageX - appCanvas.offsetLeft;
    startY = touch.pageY - appCanvas.offsetTop;
    scrollLeft = appCanvas.scrollLeft;
    scrollTop = appCanvas.scrollTop;
});

appCanvas.addEventListener('touchend', () => {
    isPanning = false;
});
...
appCanvas.addEventListener('touchmove', (e) => {
    if (!isPanning) return;
    const touch = e.touches[0];
    const x = touch.pageX - appCanvas.offsetLeft;
    const y = touch.pageY - appCanvas.offsetTop;
    const walkX = (x - startX) * 1.5;
    const walkY = (y - startY) * 1.5;
    appCanvas.scrollLeft = scrollLeft - walkX;
    appCanvas.scrollTop = scrollTop - walkY;
